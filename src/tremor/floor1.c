...
static int floor1_inverse2(vorbis_block *vb,vorbis_look_floor *in,void *memo,
  ...
  if(memo){
    ...
    /* Tail past the last floor post: apply the dB lookup the same way
       render_line does (upstream multiplied by the raw 0-255 dB index
       here). Spec-valid streams always have a post at X=n, so this loop
       only runs for degenerate floors with hx<n. ly is clamped above. */
    for(j=hx;j<n;j++)out[j]=MULT31_SHIFT15(out[j],FLOOR_fromdB_LOOKUP[ly]);
    return(1);
  }
