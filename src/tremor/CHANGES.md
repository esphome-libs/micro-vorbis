...
- **DSP setup arena in `vorbis_dsp_state`** (`block.{c,h}`, `backends.h`,
  ...
  - **Sizing** is computed once by `_vorbis_dsp_arena_compute_size()` in
    ...
    allocations 1:1. `mapping0_arena_size` owns no term of its own - it is
    exactly the sum of the floor/residue `arena_size` callbacks per submap
    (see the mapping0 look-layer elimination below); `res0_arena_size`
    (residue backends 0/1/2) and `floor1_arena_size` are each a constant `0`
    since their look-layer eliminations below - `res0_look`/`floor1_look` no
    longer allocate. floor0 is the only backend whose `arena_size` is
    nonzero (only charged for a submap that actually uses it), so
    `mapping0_arena_size` is `0` outright for an all-floor1 stream (every
    fixture in `tests/data/`). The mirror
...
- **Mapping0 look-layer elimination** (`backends.h`, `mapping0.c`): completes
  the look-layer removal above. `vorbis_look_mapping0` - a per-mode struct
  holding parallel `floor_look`/`residue_look`/`floor_func`/`residue_func`
  arrays, one entry per submap - is gone entirely: `vorbis_info_mapping0`
  itself is now the "look" (`mapping0_look` returns the info pointer
  unchanged), since floor1 and every residue backend's `look()` is already an
  identity above. `mapping0_inverse` dispatches straight off
  `ci->floor_type[floornum]`/`ci->residue_type[resnum]` into `_floor_P`/
  `_residue_P` and reads `ci->floor_param[floornum]`/`ci->residue_param[resnum]`
  as the look, fetched fresh per submap in each loop body rather than cached
  at init. floor0 is the one backend whose `look()` still computes real
  decode-time state (`linearmap`/`lsp_look`, sized by the mode's blockflag as
  well as the floor definition - see `floor0.c`, deliberately unmodified by
  this pass), so it cannot be collapsed to an identity the same way. Rather
  than reintroduce a mapping0-owned cache, `vorbis_info_floor0` gains a
  two-entry `look_cache` (indexed by blockflag: 0 = short block, 1 = long
  block); `mapping0_look` still builds it there, once per mode, exactly
  matching what `mapping0_arena_size` charges. This is safe because a
  `codec_setup_info`/`vorbis_info_floor0` is used by exactly one
  `vorbis_dsp_state` over its lifetime (one `vorbis_synthesis_init_ex` call
  per parsed header - see `ogg_vorbis_decoder.cpp`), and the setup arena the
  cached pointers reference is torn down together with the info struct
  (`vorbis_dsp_clear` then `vorbis_info_clear`), so there is no
  cross-instance dangling-pointer risk; two submaps or modes that happen to
  share the same floor definition and blockflag simply reuse (or harmlessly
  rebuild) the same cache slot. `mapping0_arena_size` no longer has any term
  of its own - it is exactly the per-submap sum of the floor/residue
  `arena_size` callbacks, which is `0` for every submap except a floor0 one;
  `mapping0_free_look` stays a no-op.
