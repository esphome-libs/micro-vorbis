...
/* ====================================================================
 * Master-shaped (full-block) backward MDCT. Renamed with a _full suffix;
 * superseded by the lowmem half-block scheme below and removed once
 * mapping0.c is ported to it.
 * ==================================================================== */

/* 8 point butterfly (in place) */
STIN void mdct_butterfly_8_full(DATA_TYPE *x){
...
STIN void mdct_butterfly_16_full(DATA_TYPE *x){
  ...
  REG_TYPE r0, r1;
	   ...
	   mdct_butterfly_8_full(x);
	   mdct_butterfly_8_full(x+8);
}

/* 32 point butterfly (in place, 4 register) */
STIN void mdct_butterfly_32_full(DATA_TYPE *x){
  ...
  REG_TYPE r0, r1;
	   ...
	   mdct_butterfly_16_full(x);
	   mdct_butterfly_16_full(x+16);
}

/* N/stage point generic N stage butterfly (in place, 2 register) */
STIN void mdct_butterfly_generic_full(DATA_TYPE *x,int points,int step){
...
STIN void mdct_butterflies_full(DATA_TYPE *x,int points,int shift){
  ...
  for(i=0;--stages>0;i++){
    for(j=0;j<(1<<i);j++)
      mdct_butterfly_generic_full(x+(points>>i)*j,points>>i,4<<(i+shift));
  }

  for(j=0;j<points;j+=32)
    mdct_butterfly_32_full(x+j);

}

/* shared by both the full-block and lowmem bitreverse permutations */
...
STIN void mdct_bitreverse_full(DATA_TYPE *x,int n,int step,int shift){
...
void mdct_backward_full(int n, DATA_TYPE *in, DATA_TYPE *out){
  int n2=n>>1;
  int n4=n>>2;
  DATA_TYPE *iX;
  DATA_TYPE *oX;
  ...
  mdct_butterflies_full(out+n2,n2,shift);
  mdct_bitreverse_full(out,n,step,shift);
...
/* ====================================================================
 * Lowmem half-block backward MDCT. Operates on n/2 values in place and
 * defers the final deinterleave/expansion + windowing/overlap-add to
 * mdct_unroll_lap(), which runs at PCM readout time against a half-block
 * work buffer and a quarter-block mdctright tail (see ivorbiscodec.h).
 * ==================================================================== */
...
/* permutation-only bit-reversal; unlike mdct_bitreverse_full this does
   not fold in the cross-product reconstruction step */
...
STIN void mdct_step8(DATA_TYPE *x, int n, int step){
  ...
  case 0:
	...
	r0  =  x[6];
	r1  = -x[7];
	/* upstream lowmem reads x+5,x+6 here, colliding with the pair
	   above and corrupting n=8192 blocks; fixed to x+6,x+7 */
	XPROD31( r0, r1, v0, v1, x+6, x+7 );
...
/* reconstruction + window + overlap-add. Emits raw s7.24 fixed-point
   samples (no clip/shift here); the wrapper rounds and clips to 16-bit
   at readout, which lets it round-to-nearest instead of floor-shift. */
void mdct_unroll_lap(int n0,int n1,
  ...
  DATA_TYPE *post;
  const LOOKUP_T *wR=(W && lW ? w1+(n1>>1) : w0+(n0>>1));
  const LOOKUP_T *wL=(W && lW ? w1         : w0        );
  ...
  wL    += off;
  while(r<post){
    /* MULT31 truncates, so -MULT31(l,w) != MULT31(-l,w); negate the
       sample before the multiply to match the master pipeline's rounding */
    *out = (MULT31(*r++,*--wR) + MULT31(-*l,*wL++));
    out+=step;
    l+=2;
  }
