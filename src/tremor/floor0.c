...
STIN ogg_int32_t vorbis_invsqlook_i(long a,long e){
  ...
  e=(e>>1)+21;
  if((unsigned long)e>=32)return(0);
  return(val>>e);
}
...
STIN ogg_int32_t vorbis_coslook2_i(long a){
  ...
  {               
    int i=a>>COS_LOOKUP_I_SHIFT;
    int d=a&COS_LOOKUP_I_MASK;
    a=(((ogg_int32_t)((ogg_uint32_t)COS_LOOKUP_I[i]<<COS_LOOKUP_I_SHIFT))-
...
static void vorbis_lsp_to_curve(ogg_int32_t *curve,int *map,int n,int ln,
  ...
  int ampi=amp;
  /* m <= 255 (8-bit order), so this alloca is at most 1020 bytes */
...
static void floor0_free_look(vorbis_look_floor *i){
  /* microVorbis: the look struct and its linearmap/lsp_look tables now live in
     the DSP setup arena (see floor0_look / floor0_arena_size), which is freed
     in one shot by vorbis_dsp_clear. Nothing to free per-look. */
  (void)i;
}
...
static vorbis_info_floor *floor0_unpack (vorbis_info *vi,oggpack_buffer *opb){
  ...
  if(info->ampbits<1 || info->ampbits>=32)goto err_out;
  if(info->numbooks<1)goto err_out;
    
  for(j=0;j<info->numbooks;j++){
    info->books[j]=oggpack_read(opb,8);
    if(info->books[j]<0 || info->books[j]>=ci->books)goto err_out;
    if(ci->book_param[info->books[j]].dec_type==0)goto err_out;
...
static vorbis_look_floor *floor0_look (vorbis_dsp_state *vd,vorbis_info_mode *mi,
  ...
  codec_setup_info   *ci=(codec_setup_info *)vi->codec_setup;
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
  vorbis_look_floor0 *look=(vorbis_look_floor0 *)_vorbis_setup_calloc(vd,1,sizeof(*look));
  look->m=info->order;
  ...
  /* the mapping from a linear scale to a smaller bark scale is
     ...
     accurate */
  look->linearmap=(int *)_vorbis_setup_alloc(vd,(look->n+1)*sizeof(*look->linearmap));
  for(j=0;j<look->n;j++){
  ...
  look->lsp_look=(ogg_int32_t *)_vorbis_setup_alloc(vd,look->ln*sizeof(*look->lsp_look));
  for(j=0;j<look->ln;j++)
...
/* microVorbis: bytes floor0_look() bumps from the DSP setup arena. Mirrors the
   allocations above 1:1; keep in sync with floor0_look. */
static long floor0_arena_size(vorbis_dsp_state *vd,vorbis_info_mode *mi,
                              vorbis_info_floor *i){
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
  codec_setup_info   *ci=(codec_setup_info *)vd->vi->codec_setup;
  long n=ci->blocksizes[mi->blockflag]/2; /* look->n */
  long size=0;
  size+=_vorbis_arena_round(sizeof(vorbis_look_floor0));
  size+=_vorbis_arena_round((n+1)*(long)sizeof(int));              /* linearmap */
  size+=_vorbis_arena_round(info->barkmap*(long)sizeof(ogg_int32_t)); /* lsp_look */
  return size;
}
...
static void *floor0_inverse1(vorbis_block *vb,vorbis_look_floor *i){
  ...
  if(ampraw>0){ /* also handles the -1 out of data case */
    long maxval=(1U<<info->ampbits)-1;
    int amp=(int)((((unsigned)ampraw*(unsigned)info->ampdB)<<4)/(unsigned long)maxval);
