...
CMAKE_SOURCE = os.path.join(ROOT, "host_examples", "vorbis_to_wav")
CMAKE_ARGS = []
...
# build_nosan matches this repo's build_*/ gitignore convention (a no-sanitizer
# build dir alongside the default build/).
EXCLUDE_SEGMENTS = {"build", "build_nosan", ".pio", "managed_components", "cmake-build"}
# Skip these repo-relative directories entirely.
# src/tremor is a forked third-party Tremor decoder, excluded from house lint
# (see script/cppcheck.sh suppressions and CLAUDE.md). lib/micro-ogg-demuxer
# is a git submodule, also excluded from house lint there.
EXCLUDE_DIRS = ["src/tremor", "lib/micro-ogg-demuxer"]
# Skip files matching any of these basename regexes (generated data headers).
EXCLUDE_BASENAMES = [r"^test_audio\.h$"]
